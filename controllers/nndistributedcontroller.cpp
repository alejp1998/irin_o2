...
void CNNDistributedController::SimulationStep(unsigned n_step_number, double f_time, double f_step_interval)
{
				...
				switch( (*j)->GetType())
				{
					...
					case SENSOR_PROXIMITY:
						...
						for ( int i = 0 ; i < (*j)->GetNumberOfInputs() ; i++)
						{
							...
							if(i == 1){
								proxS1 = pfTotalSensorInputs[i];
							}else if (i==2){
								proxS2 = pfTotalSensorInputs[i];
							}
						}
						break;
	...
	{
		printf("MOTORS: ");
		printf("%2.4f %2.4f TIME( %2.4f )\n", m_fOutputMatrix[(m_unNumberOfLayers -1 )][0], m_fOutputMatrix[(m_unNumberOfLayers -1 )][1],m_fTime);
		printf("\n");
		printf("---------------\n");
		printf("---------------\n");
	}
